...
<div class="container py-8 px-4 box-bg rounded-lg shadow-lg mx-auto mt-8">
    ...
    <div class="text-center mb-4">
        <button onclick="openCreateModal()" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Ajouter un utilisateur</button>
    </div>
        ...
        @foreach($users as $user)
            <tr class="bg-white hover:bg-gray-100 transition">
                ...
                <td class="py-4 px-6 border-b border-gray-300 flex space-x-2 justify-center">
                    <button type="button" onclick="openEditModal({{ json_encode($user) }})" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Modifier</button>
                    <button type="button" onclick="openConfirmDeleteModal({{ $user->id }})" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">Supprimer</button>
                </td>
            </tr>
        @endforeach
        </tbody>
